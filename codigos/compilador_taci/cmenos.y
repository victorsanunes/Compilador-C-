...
%token PLUS MINUS TIMES OVER EQ VIRG EQEQ MINUSEQ PLUSEQ NOTEQ LT RT LPAREN RPAREN OPENCOL CLOSECOL OPENKEY CLOSEKEY SEMI
%token ERROR
...
var_declaracao		: tipo_especificador id SEMI
			...
			| tipo_especificador id OPENCOL fator CLOSECOL SEMI
			{
				$$ = newDeclNode(VetK);
				$$->escopo=nomeEscopo;
				$$->tipo = $1->tipo;
...
composto_decl		: OPENKEY local_declaracoes statement_lista CLOSEKEY
			{ YYSTYPE t = $2;
			  if (t != NULL)
			   { while (t->sibling != NULL)
			      t = t->sibling;
...
relacional		:  MINUSEQ { $$ = newExpressionNode (OpK);
			$$->attr.op = MINUSEQ;}
			    | LT {$$ = newExpressionNode (OpK);
					$$->attr.op = LT;}
			    | RT {$$ = newExpressionNode (OpK);
					$$->attr.op = RT;}
			    | PLUSEQ {$$ = newExpressionNode (OpK);
					$$->attr.op = PLUSEQ;}
			    | EQEQ { $$ = newExpressionNode (OpK);
					$$->attr.op = EQEQ;}
			    | NOTEQ { $$ = newExpressionNode (OpK);
					$$->attr.op = NOTEQ;}
			    ;
